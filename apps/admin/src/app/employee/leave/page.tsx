...
export default async function EmployeeLeavePage() {
  ...
  const [{ data: types }, { data: rows }, { data: balances }] = await Promise.all([
    ...
    emp
      ? supabase
          .from("leave_requests")
          .select("id, start_date, end_date, status, days_count, reason, leave_types(name_ckb)")
    ...
    emp
      ? supabase
          .from("leave_balances")
          .select(
            "entitled_days, used_days, remaining_days, leave_types(name_ckb)",
          )
          .eq("employee_id", emp.id)
          .eq("year", year)
      : Promise.resolve({ data: [] }),
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-ink-muted">داواکاری لەلایەن ئەدمینەوە پەسەند دەکرێت</p>
      </div>
            ...
            return (
              <div key={r.id} className="panel p-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="rounded-lg bg-surface-muted px-2 py-1 text-xs">{r.status}</span>
                </div>
